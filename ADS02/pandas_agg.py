...
def get_prices_for_heaviest_item(inventory):
    ...
    if inventory.shape[0] == 0:
        ...
        inventory = inventory[inventory.in_stock]
        new = inventory[['price', 'weight']].groupby(inventory.category).max().sort_values(by='weight', ascending=False)
        index = new.index.values
        values = new.price.values
        series = pd.Series(index=index, data=values)
        return series
...
def reshape_temperature_data(measurements):
    ...
    import pandas as pd
    df2 = pd.melt(measurements, id_vars=["location"], var_name="date").sort_values(by=['location', 'value'])
