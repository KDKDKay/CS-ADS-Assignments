def nan_processor(df, replacement_str):
    ...
    df = df.replace(replacement_str, np.nan).dropna()
    return df
...
def feature_cleaner(df, low, high):
    ...
    df[df <= df.quantile(q=low)] = np.nan
    df[df >= df.quantile(q=high)] = np.nan
    df = df.dropna()
...
def get_feature(df):
    ...
    for column in df0.columns:
        df0_ratio = (max(df0[column]) - min(df0[column]))/df0[column].var()
        rclass0.append(df0_ratio)
    for column in df1.columns:
        df1_ratio = (max(df1[column]) - min(df1[column]))/df1[column].var()
        rclass1.append(df1_ratio)

    k = []
    for r0var, r1var in zip(rclass0, rclass1):
        k.append(max(r0var, r1var) / min(r0var, r1var))
    _ratio, fcol = max(zip(k, columns))
    return fcol
...
def one_hot_encode(label_to_encode, labels):
    ...
    hot_encode = [1 if label_to_encode == label else 0 for label in labels]
    return hot_encode
