...
def diff_in_days(df):
    ...
    df['difference_days'] = (abs(df['time_1'] - df['time_2'])).astype('timedelta64[D]')
    final = df.drop(['time_1', 'time_2'], axis=1)
    return final
...
def return_location(df):
    ...
    for index in range(len(df)):
        row_str = json.loads(str(df['locations'][index]))
        all_rows.append(row_str)

    short_name_list = []
    for element in all_rows:
        s_n = element.get("short_name")
        short_name_list.append(s_n)

    final_df = pd.DataFrame(short_name_list, columns=["short_name"])
    return final_df
